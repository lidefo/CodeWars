...
It uses a pure Math based solution that gives consistent results no matter what order a set of ranked activities are
completed at.
...
class User:
    ...
    def inc_progress(self, kata_rank):
        ...
        if self.rank < 8:
            ...
            elif ranks.index(kata_rank) + 1 == self.rank_index:
                self.progress += 1

            elif kata_rank > self.rank:
                self.progress += 10 * (ranks.index(kata_rank) - self.rank_index) ** 2
