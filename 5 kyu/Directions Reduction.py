...
def dirReduc(arr):
    d = {'w': 'WEST', 'e': 'EAST', 'n': 'NORTH', 's': 'SOUTH'}
    firstsymb = ''
    for c in arr:
        firstsymb += c[0].lower()
    for _ in range(len(firstsymb)):
        firstsymb = firstsymb.replace('we', '').replace('ew', '').replace('ns', '').replace('sn', '')
    return [d[c] for c in firstsymb]
