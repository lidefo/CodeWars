...
So the maximum for 'a' in s1 and s2 is 4 from s1; the maximum for 'b' is 3 from s2. In the following we will
not consider letters when the maximum of their occurrences is less than or equal to 1.
...
def mix(s1: str, s2: str):
    s1_chars = [char for char in s1 if char.isalpha() and char.islower()]
    s2_chars = [char for char in s2 if char.isalpha() and char.islower()]
    chars = set(s1_chars)
    chars.update(s2_chars)

    out = []

    for char in chars:
        if s1.count(char) > 1 or s2.count(char) > 1:
            if s1.count(char) == s2.count(char):
                prefix = '=:'
            else:
                prefix = ['1', '2'][s2.count(char) > s1.count(char)] + ':'
            out.append(prefix + char * max(s2.count(char), s1.count(char)))
